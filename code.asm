...
genPalindrome PROC
    ; Устанавливаем направление обработки цепочек
    CLD                         ; Устанавливаем направление: вперед

    ; Копируем строку в начало результата
    ...
    ; Переворачиваем строку и добавляем к результату
    lea rsi, buffer             ; Адрес исходной строки
    add rsi, qword ptr [lens]   ; Указатель на конец исходной строки
    dec rsi                     ; Переходим к последнему символу
    mov rcx, qword ptr [lens]   ; Количество символов в строке
    ...
    add rdi, qword ptr [lens]   ; Переход к концу первой копии строки
    add rdi, qword ptr [lens]   ; Переход к концу второй копии строки
